...
A custom Home Assistant integration for SVK LMC320 heat pump controllers, providing comprehensive monitoring and control capabilities through the modern JSON API interface.
...
* **197+ Entities**: Comprehensive monitoring of all heat pump parameters
* **JSON API Support**: Modern JSON API for efficient data communication
...
The integration uses a modern catalog-driven architecture with an entity factory pattern:

* **Entity Catalog**: Centralized entity definitions in ``catalog.py`` provide:
  * Complete entity metadata and configuration
  * Default entity selection for new installations
  * Type-safe entity ID mapping and validation
  
* **Entity Factory Pattern**: Dynamic entity creation with:
  * Automatic entity instantiation based on catalog definitions
  * Consistent entity behavior across all platforms
  * Simplified maintenance and extensibility
  
* **Benefits**:
  * Easy addition of new entities without code changes
  * Consistent entity handling and validation
...
The integration uses the modern JSON API for optimal performance:
...
* **Smart Chunking**: Automatically splits large requests into optimal chunks
  * Fixed chunk size: 25 entities per request for optimal server compatibility
...
* **Parallel Processing**: Concurrent network operations with semaphore control
...
The integration includes robust data validation for consistent handling:

* **Temperature Sentinel Rule**: Values ≤ -80.0°C mark entities as unavailable
* **Percentage Clamping**: Automatic scaling of percentage values to 0-100 range
* **Last-Good Value Preservation**: Maintains previous values for temporarily unavailable entities
* **Unified Entity Availability**: Consistent availability checks across all entities
...
The integration provides 197+ entities organized into the following categories:
...
* **Solutions**:
  ...
  3. Test authentication with curl: ``curl --digest -u admin:password http://192.168.1.100/cgi-bin/json_values.cgi``
...
* **Entity Selection**: Enable only needed entities
  ...
  * **Full Monitoring**: All 197+ entities for comprehensive oversight
...
* Expanded entity catalog to 197+ entities
...
* Implemented temperature sentinel value handling (≤ -80.0°C marks entities unavailable)
* Added entity availability validation
* Added percentage clamping feature (0-100 range)
* Implemented catalog-driven architecture with entity factory pattern