...
from .catalog import SWITCH_ENTITIES, ENTITIES
...
class SVKSwitch(SVKBaseEntity, SwitchEntity, CoordinatorEntity):
    ...
    def available(self) -> bool:
        ...
        if self.coordinator.is_json_client:
            ...
            value = self.coordinator.get_entity_value(self._entity_key)
            _LOGGER.info("JSON API Switch %s availability: %s (entity exists in mapping, current value: %s)",
            ...
            _LOGGER.debug("HTML API Switch %s availability: %s (last_update_success: %s, writes_enabled: %s, entity_available: %s)",
...
class SVKHeatpumpSwitch(SVKHeatpumpBaseEntity, SwitchEntity):
    ...
    def __init__(
        ...
        _LOGGER.debug("Creating switch entity: %s (ID: %s, enabled_by_default: %s)",
    ...
    def is_on(self) -> Optional[bool]:
        ...
        # Log value retrieval for debugging
        if value is None:
            _LOGGER.debug("Switch %s (ID: %s) returned None value", self._entity_key, self._entity_id)
        else:
            _LOGGER.debug("Switch %s (ID: %s) returned value: %s", self._entity_key, self._entity_id, value)
...
async def async_setup_entry(
    ...
    _LOGGER.info("Setting up SVK Heatpump switch entities for entry %s", config_entry.entry_id)
    ...
    if coordinator.is_json_client:
        ...
        for entity_key in SWITCH_ENTITIES:
            try:
                ...
                _LOGGER.debug("Added switch entity: %s (name: %s, enabled_by_default: %s)",
        ...
        for entity_key, entity_id in essential_entities:
            try:
                ...
                switch_entities.append(switch)
                _LOGGER.info("Added fallback switch entity: %s (ID: %s)", entity_key, entity_id)
            except Exception as err:
                _LOGGER.error("Failed to create fallback switch entity %s (ID: %s): %s",
