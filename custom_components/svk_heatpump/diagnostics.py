...
async def async_get_config_entry_diagnostics(
    ...
    diagnostics_data = {
        "config_entry_data": _redact_auth_data(async_redact_data(config_entry.data, TO_REDACT)),
    ...
    if coordinator.data:
        ...
        diagnostics_data["parsed_data"] = _redact_auth_data(async_redact_data(data_copy, TO_REDACT))
...
def get_html_debug_info(coordinator) -> Dict[str, Any]:
    ...
    debug_info["parsing_stats"] = {
        ...
        "temperature_sensors": len([k for k in coordinator.data.keys() if "temp" in k.lower()]) if coordinator.data else 0,
        "performance_metrics": len([k for k in coordinator.data.keys() if any(metric in k.lower() for metric in ["speed", "capacity", "runtime"])]) if coordinator.data else 0,
        "system_info": len([k for k in coordinator.data.keys() if any(info in k.lower() for info in ["ip", "version", "log"])]) if coordinator.data else 0,
    }
...
def create_diagnostics_report(hass: HomeAssistant, config_entry: ConfigEntry) -> Dict[str, Any]:
...
def count_entities_by_type(coordinator) -> Dict[str, Any]:
    ...
    return {
        ...
        "availability_percentage": {
            entity_type: round((available_counts[entity_type] / entity_counts[entity_type] * 100) if entity_counts[entity_type] > 0 else 0, 1)
